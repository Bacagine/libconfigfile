    ...
    switch ( astConfFile[ii].iDataType ) {
      case DATATYPE_BOOL: {
        if ( !strcmp(astConfFile[ii].pszDefaultValue, "false") ||
          ...
          *(bool *) astConfFile[ii].pData = false;
        }
        ...
        if ( !strcmp(astConfFile[ii].pszDefaultValue, "true")  ||
          ...
          *(bool *) astConfFile[ii].pData = true;
        }
        break;
      }
      case DATATYPE_CHAR: {
...
bool bReadConfFile(const char *kpszFileName, PSTRUCT_CONF_FILE astConfFile) {
  FILE *fpFile = NULL;
  char szLine[1024] = "";
  int ii = 0;
  ...
  while ( fgets(szLine, sizeof(szLine), fpFile) ) {
    ...
    for ( ii = 0; astConfFile[ii].pszVariableName; ii++ ) {
      if ( !strcmp(pTok, astConfFile[ii].pszVariableName) ) {
        ...
        switch ( astConfFile[ii].iDataType ) {
          case DATATYPE_BOOL: {
            if      ( !strcmp(pTok, "false") || !strcmp(pTok, "FALSE") || !strcmp(pTok, "0") ) *(bool *) astConfFile[ii].pData = false;
            else if ( !strcmp(pTok, "true")  || !strcmp(pTok, "TRUE")  || !strcmp(pTok, "1") ) *(bool *) astConfFile[ii].pData = true;
            else    return false;
            break;
          }
          case DATATYPE_CHAR: {
